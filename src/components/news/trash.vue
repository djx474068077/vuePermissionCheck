<template>
  <div class="news-list">
    <Table :columns="columns" :data="news" size="small" ref="table"></Table>
...
<script>
  ...
  export default {
            ...
            render: (h, params) => {
              return [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.reviewNews(this.news[params.index].id)
                    }
                  }
                }, '恢复'),
                h('Button', {
    ...
    methods: {
      listNews (page) {
        this.$http.get('/api/news/trash?p=' + page).then((response) => {
      ...
      deleteNews (index) {
        this.$Modal.confirm({
          ...
          cancelText: '取消',
          onOk: () => {
            this.$http.post('/api/news/realDelete', qs.stringify({
              id: index
            })).then((response) => {
              let res = response.data
              if (res.status === 10000) {
      ...
      reviewNews (index) {
        this.$Modal.confirm({
          ...
          cancelText: '取消',
          onOk: () => {
            this.$http.post('/api/news/recover', qs.stringify({
              id: index
            })).then((response) => {
              let res = response.data
              if (res.status === 10000) {
