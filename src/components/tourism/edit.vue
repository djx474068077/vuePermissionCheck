    ...
    <Form :model="formItem" :label-width="80">
      ...
      <FormItem label="选择日期">
        <Row>
          <Col span="11">
          <FormItem prop="date">
            <DatePicker type="datetime"  @change="changeStartTime" format="yyyy-MM-dd HH:mm:ss" placeholder="选择开始日期" v-model="formItem.start_time"></DatePicker>
          </FormItem>
          </Col>
          <Col span="2" style="text-align: center">---</Col>
          <Col span="11">
          <FormItem prop="time">
            <DatePicker type="datetime" @change="changeEndTime" format="yyyy-MM-dd HH:mm:ss"  placeholder="选择结束日期" v-model="formItem.end_time"></DatePicker>
          </FormItem>
          </Col>
        </Row>
      </FormItem>
      ...
      <Form-item label="内容介绍">
        <ueditor :value="formItem.value" :config="config" ref="ueditor" v-on:ready="editorReady"></ueditor>
      </Form-item>
      <FormItem>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit () {
        this.$http.post('/api/tourism/edit', qs.stringify({
      ...
      tourismDetail (id) {
        this.$http.get('/api/tourism/detail?id=' + id).then((response) => {
          let res = response.data
          if (res.status === 10000) {
            this.tourism = res.data
            this.formItem.title = this.tourism.Title
            this.formItem.summary = this.tourism.Summary
            this.formItem.value = this.tourism.Content
            this.formItem.price = this.tourism.Price
            this.formItem.child_price = this.tourism.ChildPrice
            this.formItem.adult_price = this.tourism.AdultPrice
            this.formItem.start_time = this.tourism.StartTime
            this.formItem.end_time = this.tourism.EndTime
            this.formItem.user_limit = this.tourism.UserLimit
            this.formItem.cover = this.tourism.Cover
            this.formItem.insurance = this.tourism.Insurance.Id
            this.$refs.ueditor.setContent(this.tourism.Content)
          }
        }).catch(() => {
          this.$Message.error('通讯错误，请重试')
        })
